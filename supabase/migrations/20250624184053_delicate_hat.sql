/*
  # Fixed Chat System Migration

  1. Create Tables First
    - `chat_rooms` - Chat room containers
    - `chat_participants` - Users in each chat room  
    - `chat_room_messages` - Messages within chat rooms

  2. Enable RLS
    - Enable Row Level Security on all tables

  3. Create Policies
    - Secure access to chat data based on participation

  4. Create Functions and Triggers
    - Chat management functions
    - Automatic chat creation triggers
*/

-- ============================================================================
-- 1. CREATE TABLES FIRST (with IF NOT EXISTS to avoid conflicts)
-- ============================================================================
...
-- ============================================================================
-- 3. CREATE POLICIES (with existence checks to avoid conflicts)
-- ============================================================================
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'chat_participants' AND policyname = 'Users can join chats they have access to'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'chat_room_messages' AND policyname = 'Users can view messages in their chat rooms'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'chat_room_messages' AND policyname = 'Users can send messages to their chat rooms'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_chat_room_messages_chat_room_id ON chat_room_messages(chat_room_id);
...
-- ============================================================================
-- 5. CREATE FUNCTIONS (after all tables and policies exist)
-- ============================================================================
...
CREATE OR REPLACE FUNCTION handle_task_assignment_chat(p_task_id uuid, p_assignee_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION handle_project_creation_chat(p_project_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- ============================================================================
-- 6. CREATE TRIGGERS (after all functions exist)
-- ============================================================================

-- Trigger function for task assignment
CREATE OR REPLACE FUNCTION trigger_task_assignment_chat()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF NEW.assignee_id IS NOT NULL AND (OLD.assignee_id IS NULL OR OLD.assignee_id != NEW.assignee_id) THEN
    PERFORM handle_task_assignment_chat(NEW.id, NEW.assignee_id);
  END IF;
...
CREATE OR REPLACE FUNCTION trigger_project_creation_chat()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  PERFORM handle_project_creation_chat(NEW.id);
...
CREATE TRIGGER task_assignment_chat_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION trigger_task_assignment_chat();

CREATE TRIGGER project_creation_chat_trigger
  AFTER INSERT ON projects
  FOR EACH ROW
  EXECUTE FUNCTION trigger_project_creation_chat();

-- ============================================================================
-- 7. GRANT PERMISSIONS (after all functions exist)
-- ============================================================================
...
GRANT EXECUTE ON FUNCTION handle_task_assignment_chat(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION handle_project_creation_chat(uuid) TO authenticated;