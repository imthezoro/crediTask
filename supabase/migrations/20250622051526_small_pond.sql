/*
  # Application Buckets and Auto-Assignment System

  1. New Tables
    - `application_buckets` - Groups applications per task for better management
    - `auto_assignment_timers` - Manages auto-assignment timing and windows

  2. Enhanced Tables
    - Add `application_window_minutes` to tasks
    - Add `bucket_id`, `selected`, `reviewed` to task_applications

  3. Functions
    - Application bucket management
    - Auto-assignment scoring and evaluation
    - Timer management for application windows
...
CREATE OR REPLACE FUNCTION get_or_create_application_bucket(p_task_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION evaluate_auto_assignment(p_task_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION start_auto_assignment_timer(p_task_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION extend_auto_assignment_timer(p_task_id uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
